...
RUN printf "[Unit] \nDescription=collector-pingdom Service \nAfter=network.target \n[Service] \nType=simple \nUser=root \nExecStart=/bin/java -jar /opt/dashboard/hygieia-uptime-pingdom-collector/target/pingdom-uptime-collector.jar --spring.config.location=application.properties \nRestart=on-abort \n[Install] \nWantedBy=multi-user.target \n" > /etc/systemd/system/collector-pingdom.service
