...
# now using angular 
RUN git clone https://github.com/Hygieia/UI.git
RUN cd /opt/dashboard/UI && echo N | npm install 
RUN cd /opt/dashboard/UI && npm install -g @angular/cli@8.0.3
RUN cd /opt/dashboard/UI && ng version
RUN cd /opt/dashboard/UI && yarn upgrade
RUN cd /opt/dashboard/UI && cp proxy.conf.json backup-proxy.conf.json
RUN cd /opt/dashboard/UI && sed -i 's/8080/8084/g' proxy.conf.json
...
WORKDIR /opt/dashboard/collectors

RUN git clone https://github.com/Hygieia/hygieia-scm-bitbucket-collector.git
RUN cd /opt/dashboard/collectors/hygieia-scm-bitbucket-collector && mvn install

RUN git clone https://github.com/Hygieia/hygieia-publisher-jenkins-plugin.git
RUN cd /opt/dashboard/collectors/hygieia-publisher-jenkins-plugin && mvn clean package

RUN git clone https://github.com/Hygieia/hygieia-misc-score-collector.git
RUN cd /opt/dashboard/collectors/hygieia-misc-score-collector && mvn install

RUN git clone https://github.com/Hygieia/hygieia-codequality-sonar-collector.git
RUN cd /opt/dashboard/collectors/hygieia-codequality-sonar-collector && mvn install

RUN git clone https://github.com/Hygieia/hygieia-feature-jira-collector.git
RUN cd /opt/dashboard/collectors/hygieia-feature-jira-collector && mvn install

RUN git clone https://ysbre:n9EBMz2uCbmKVCstguCD@bitbucket.org/lumsb-hygieia/hygieia-uptime-pingdom-collector.git
RUN cd /opt/dashboard/collectors/hygieia-uptime-pingdom-collector && mvn install
...
RUN printf "[Unit] \nDescription=collector-sonar Service \nAfter=network.target \n[Service] \nType=simple \nUser=root \nExecStart=/bin/java -jar /opt/dashboard/collectors/hygieia-codequality-sonar-collector/target/sonar-codequality-collector.jar --spring.config.name=sonar --spring.config.location=/opt/dashboard/collectors/sonar.properties -Dlogging.config=file:/opt/dashboard/collectors/hygieia-codequality-sonar-collector/src/main/resources/logback.xml \nRestart=on-abort \n[Install] \nWantedBy=multi-user.target \n" > /etc/systemd/system/collector-sonar.service

RUN printf "[Unit] \nDescription=collector-score Service \nAfter=network.target \n[Service] \nType=simple \nUser=root \nExecStart=/bin/java -jar /opt/dashboard/collectors/hygieia-misc-score-collector/target/score-collector.jar --spring.config.name=score --spring.config.location=/opt/dashboard/collectors/score.properties -Dlogging.config=file:/opt/dashboard/collectors/hygieia-misc-score-collector/src/main/resources/logback.xml \nRestart=on-abort \n[Install] \nWantedBy=multi-user.target \n" > /etc/systemd/system/collector-score.service

RUN printf "[Unit] \nDescription=collector-bitbucket Service \nAfter=network.target \n[Service] \nType=simple \nUser=root \nExecStart=/bin/java -jar /opt/dashboard/collectors/hygieia-scm-bitbucket-collector/target/bitbucket-scm-collector.jar --spring.config.name=git --spring.config.location=/opt/dashboard/collectors/bitbucket.properties -Dlogging.config=file:/opt/dashboard/collectors/hygieia-scm-bitbucket-collector/src/main/resources/logback.xml \nRestart=on-abort \n[Install] \nWantedBy=multi-user.target \n" > /etc/systemd/system/collector-bitbucket.service

RUN printf "[Unit] \nDescription=devopsdash-exec-api Service \nAfter=network.target \n[Service] \nType=simple \nUser=root \nExecStart=/bin/java -jar /opt/dashboard/execdashboard/exec-api/target/exec-api.jar --spring.config.location=/opt/dashboard/exec-api.properties \nRestart=on-abort \n[Install] \nWantedBy=multi-user.target \n" > /etc/systemd/system/devopsdash-exec-api.service
...
RUN printf "[Unit] \nDescription=collector-pingdom Service \nAfter=network.target \n[Service] \nType=simple \nUser=root \nExecStart=/bin/java -jar /opt/dashboard/collectors/hygieia-uptime-pingdom-collector/target/pingdom-uptime-collector.jar --spring.config.location=/opt/dashboard/collectors/pingdom.properties \nRestart=on-abort \n[Install] \nWantedBy=multi-user.target \n" > /etc/systemd/system/collector-pingdom.service
